...
function renderPlanCells(r) {
  ...
  const tagChip = (label) => `<span style="display:inline-block;background:var(--surface-2);border:1px solid var(--border);border-radius:4px;padding:0 5px;font-size:11px;color:var(--text-2);font-weight:600;margin-right:4px;">${label}</span>`;
  ...
  if (!p.matched) {
    ...
    if (p.structured) {
      tobeCell = `<div class="cell-text" style="font-size:13px;">` +
        (p.impl ? `<div>${tagChip('구현')}${p.impl}</div>` : '') +
        (p.flow ? `<div style="margin-top:5px;">${tagChip('작동')}${p.flow}</div>` : '') +
        (p.out ? `<div style="margin-top:5px;">${tagChip('산출물')}${p.out}</div>` : '') +
        `</div>`;
    } else {
